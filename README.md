...
ProofReceipt closes that gap with a single idea: **the journal of the proof binds the buyer's input.** The seller's zkVM program commits `sha256(input) ‖ verdict`; the on-chain verifier only accepts a proof whose journal matches the agreed image id and the buyer's pinned input hash. A passing verification is therefore unforgeable evidence that *this exact program ran on this exact input*. That verification is what settles the payment.
